  ...
  void mapEventToState(
    ...
    if (event is GetPaymentMethodEvent) {
      try {
        ...
        if (model != null) {
          if(model.acquirers!=null){
            emit( GetPaymentMethodSuccess(model));
          }else {
            emit(PaymentReviewScreenError(model.message??''));
          }
      ...
      try {
        var model = await repository?.placeOrder(event.paymentRefrence , event.transactionId, event.paymentStatus ,event.token, event.stripeCustomerId);
        if (model != null) {
